"""
compare_isaac_sim.py - Compare Baseline vs Petiole Lock in Isaac Sim

Loads both USD files side-by-side in Isaac Sim for visual comparison.

Run with: ~/isaacsim/python.sh src/exporterV2/core/optimizations/tests/4_petiole_lock/compare_isaac_sim.py
"""
...
def add_text_labels(stage):
    """Add text labels above each plant."""
    # Note: Text prims in USD are complex, simplified here
    # In practice, you'd use Isaac Sim UI overlays
    pass
...
def main():
    ...
    print("="*70)
    print("  Isaac Sim - Petiole Lock Comparison")
    print("="*70)
    ...
    locked_path = os.path.join(usd_dir, "petiole_lock.usda")
    ...
    print(f"  ✓ Petiole Lock at x=+1.0m")
    ...
    print("="*70)
    print("\nScene Layout:")
    print("  Left  (x=-1.0): Baseline - Articulated petiolules (D6)")
    print("  Right (x=+1.0): Petiole Lock - Fixed petiolules (Fixed)")
    print("\nInstructions:")
    print("  1. Press PLAY button to start simulation")
    print("  2. Observe petiolule movement:")
    print("     - Baseline: Petiolules oscillate/sway")
    print("     - Petiole Lock: Petiolules remain rigid")
    ...
    print("  • Physics: Petiole lock more stable, less oscillation")
    print("  • Performance: Petiole lock faster (fewer DOF)")

...
1. **Sequential stages** (`run_visual_test.py`) — 6 USD che mostrano l'effetto cumulativo delle tecniche applicate in sequenza
...
```bash
~/isaacsim/python.sh -m isaacsim 'src/exporterV2/core/optimizations/tests/visual_validation/usd_output/0_baseline.usda'
```
...
> I valori esatti vengono stampati a console durante `run_visual_test.py`.
...
```bash
~/isaacsim/python.sh src/exporterV2/core/optimizations/tests/visual_validation/load_combination_isaacsim.py --combo 1
```

**Test non-visuali:**
```bash
uv run pytest src/exporterV2/core/optimizations/tests/visual_validation/test_combinations.py -v
```

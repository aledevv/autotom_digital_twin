"""
generate_tree.py - Tree USD Generator

Generates an articulated tree USD stage from a BRANCHES configuration.
...
Key design:
- All links live directly under /World/Stem (flat hierarchy)
- Links use only AddTranslateOp (world-space position)
- Orientation/angle information lives in joint LocalPos0/LocalRot0
...
Run standalone (no Isaac Sim required):
    python -m exporterV2.generate_tree
"""
...
if __name__ == "__main__" or "exporterV2" not in sys.modules:
    # Direct execution or first import - use relative to script location
...
def get_output_usd_path() -> str:
    ...
    # Navigate from exporterV2 → src → project_root
    script_dir = os.path.dirname(os.path.abspath(__file__))
...
def _add_collision_filter(stage, body_a_path: str, body_b_path: str) -> None:
    """
...
def _parse_link_number(link_path: str) -> tuple:
    """
...
def _add_attachment_collision_filters(stage, child_link_path: str, parent_link_path: str) -> None:
    """
    ...
    """
    # Filter parent link
    _add_collision_filter(stage, child_link_path, parent_link_path)
    
    # Find and filter next link in parent chain
    branch_id, link_num = _parse_link_number(parent_link_path)
    if branch_id and link_num:
        ...
        if next_link_prim and next_link_prim.IsValid():
            _add_collision_filter(stage, child_link_path, next_link_path)
    ...
    for sibling_links in attachment_map.values():
        ...
        for i, link_a in enumerate(sibling_links):
            for link_b in sibling_links[i+1:]:
                _add_collision_filter(stage, link_a, link_b)
                _add_collision_filter(stage, link_b, link_a)
                filtered_count += 2
...
def create_internal_joint(
    ...
    _add_collision_filter(stage, child_path, parent_path)
...
def create_attachment_joint(
    ...
    _add_attachment_collision_filters(stage, child_link_path, parent_link_path)
...
def build_stage(output_path: str, branches=None, locked_joints: bool = False, skip_limit_check: bool = False):
    ...
    for b in branches:
        ...
        if is_root:
            ...
            # Compute radial offset for attachment
...
def main():
    """Generate tree USD with default configuration."""

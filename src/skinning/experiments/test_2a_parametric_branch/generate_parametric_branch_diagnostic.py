...
OUTPUT_USD = str(OUTPUT_DIR / "test_2a_parametric_branch.usda")
...
def create_rigid_link(stage, spec, centerline, physics, stem_path, index) -> str:
    ...
    s_mid = 0.5 * (physics["node_arc"][index] + physics["node_arc"][index + 1])
    collider_radius = radius_for_arc(spec, centerline, s_mid) * spec.collider_radius_scale

    collider = UsdGeom.Cylinder.Define(stage, f"{path}/Collider")

    COLLIDER_LENGTH_SCALE = 0.75
    collider_length = length * COLLIDER_LENGTH_SCALE

    collider.CreateHeightAttr().Set(collider_length)
    collider.CreateRadiusAttr().Set(collider_radius)
    collider.CreateAxisAttr().Set("Z")

    UsdGeom.Xformable(collider.GetPrim()).AddTranslateOp().Set(
        Gf.Vec3d(0.0, 0.0, length / 2.0)
    )
    ...
    mass_value = max(spec.linear_density_kg_per_m * length, 1e-5)
    UsdPhysics.MassAPI.Apply(xf.GetPrim()).CreateMassAttr().Set(mass_value)
    return path
...
def build_stage(output_path: str = OUTPUT_USD) -> str:
    ...
    print("=" * 80)
    print("TEST 2A — PARAMETRIC BRANCH GENERATION")
    print("=" * 80)
    ...
    print()
    print("Main Test 2A check:")
    print("  change BranchSpec.physics_links from 6 -> 4 or 8")
    print("  and the complete physics/skeleton discretization must regenerate.")
    print("=" * 80)
    return output_path

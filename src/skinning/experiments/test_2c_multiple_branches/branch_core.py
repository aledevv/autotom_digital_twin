...
def create_junction_joint(
    ...
    """
    Connect child_branch link 0 to an arbitrary point INSIDE a parent rigid link.

    Both local joint frames are derived from one common joint world frame,
    so the two frames coincide exactly in the authored rest pose.
    """
    parent_path = (
        parent_branch
        .link_paths[
    ...
    # Use the parent rigid-link orientation as the common joint orientation.
    common_rotation = (
        parent_branch
        .physics[
            "rotations"

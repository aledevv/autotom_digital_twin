...
OUTPUT_USD = str(
    OUTPUT_DIR / "test_2ca_single_junction_fixed.usda"
)
...
def build_stage(
    ...
    # CRITICAL:
    # PhysxArticulationAPI only configures PhysX-specific articulation
    # parameters. The USD articulation itself must also be declared.
    # This matches exporterV2/core/usd/stage.py.
    ...
    print(
        "  collision filtered : parent links "
        + ", ".join(
            str(i + 1)
            for i in junction_info[
    ...
    print(
        "  articulation root   : "
        "/World/PlantPhysics [UsdPhysics.ArticulationRootAPI]"
    )
    print()
    print("IMPORTANT:")
    print(

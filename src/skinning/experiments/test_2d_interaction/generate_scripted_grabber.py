...
OUTPUT_USD = str(
    OUTPUT_DIR / "test_2da_scripted_grabber.usda"
)
...
# ============================================================================
# SCRIPTED PHYSICAL GRABBER
# ============================================================================

GRABBER_PATH = "/World/Grabber"
GRABBER_JOINT_PATH = "/World/Grabber/GrabJoint"

# Grab the middle lateral rigid link.
GRAB_LINK_INDEX = 1
GRAB_LINK_PATH = LATERAL.link_paths[
    GRAB_LINK_INDEX
]

GRAB_LINK_LENGTH = float(
    LATERAL.physics[
        "lengths"
    ][GRAB_LINK_INDEX]
)

GRAB_LOCAL_POINT = Gf.Vec3d(
    0.0,
    0.0,
    0.50 * GRAB_LINK_LENGTH,
)

GRAB_LINK_BIND_WORLD = (
    LATERAL.physics[
        "bind"
    ][GRAB_LINK_INDEX]
)
...
GRAB_START_WORLD = (
    GRAB_LINK_BIND_WORLD.Transform(
        GRAB_LOCAL_POINT
    )
)
...
# Spring used only for user interaction.
# Translation is compliant; rotation remains free.
GRAB_TRANSLATION_STIFFNESS = 25.0  # N/m
GRAB_TRANSLATION_DAMPING = 1.0     # N*s/m


def _set_exclude_from_articulation(
    joint,
):
    """
    Isaac/PhysX supports physics:excludeFromArticulation on joints.

    Use the schema method when exposed by the installed USD version,
    otherwise author the standard USD attribute directly.
    """
...
def build_grabber(
    ...
    """
    Create a collision-free kinematic handle outside the plant articulation.

    A maximal-coordinate D6 joint connects the handle to one articulation link.
    Only transX/Y/Z have spring drives. Rotations are deliberately left free.
    """
    handle = UsdGeom.Xform.Define(
        stage,
        GRABBER_PATH,
    )
    ...
    rigid = UsdPhysics.RigidBodyAPI.Apply(
        handle.GetPrim()
    )

    rigid.CreateRigidBodyEnabledAttr().Set(
        True
    )

    # Kinematic bodies are driven by authored/user poses.
    rigid.CreateKinematicEnabledAttr().Set(
        True
    )

    # Visible marker only: intentionally NO CollisionAPI.
    marker = UsdGeom.Sphere.Define(
        stage,
        f"{GRABBER_PATH}/Marker",
    )

    marker.CreateRadiusAttr().Set(
        0.012
    )
    ...
    # Handle origin is exactly the grab point.
    ...
    # Grab the midpoint of the selected rigid link.
    ...
    # CRITICAL: keep the kinematic handle OUTSIDE the reduced-coordinate
    # articulation. This joint is solved as an external/maximal constraint.
    _set_exclude_from_articulation(
        joint
    )

    joint.CreateJointEnabledAttr().Set(
        True
    )

    # No translation limits are authored: the D6 translations are free,
    # but spring drives pull their relative displacement toward zero.
    for axis in (
        "transX",
        "transY",
        "transZ",
        ...
        drive.CreateStiffnessAttr().Set(
            GRAB_TRANSLATION_STIFFNESS
        )

        drive.CreateDampingAttr().Set(
            GRAB_TRANSLATION_DAMPING
        )
    ...
    return {
        "handle_path": GRABBER_PATH,
        "joint_path": GRABBER_JOINT_PATH,
        "link_path": GRAB_LINK_PATH,
    }
...
def build_stage(
    ...
    print(
        f"  handle             : "
        f"{grabber_info['handle_path']}"
    )
    print(
        f"  grabbed link       : "
        f"{grabber_info['link_path']}"
    )
    print(
        f"  spring K           : "
        f"{GRAB_TRANSLATION_STIFFNESS:.2f} N/m"
    )
    print(
        f"  spring D           : "
        f"{GRAB_TRANSLATION_DAMPING:.2f} N*s/m"
    )
    print(
        "  rotations          : FREE"
    )
    print(
        "  joint articulation : EXCLUDED"
    )
    print()
    print(
        "The orange handle has NO collider."
    )
    print(
        "The hidden branch capsules remain the physical collision proxy."
    )
    print("=" * 84)

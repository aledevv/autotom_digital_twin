"""
generate_compound_capsule.py — Test 2B-B
...
SYNC_PRESET = os.environ.get("SYNC_PRESET", "A").strip().upper()

PRESETS = {
    # A: baseline, already observed as well synchronized.
    "A": {
        "physics_links": 3,
        "samples_per_control_segment": 12,  # 3 segments * 12 + 1 = 37 rings
        "radial_segments": 14,
    },

    # B: increase only visual/skinning workload.
    "B": {
        "physics_links": 3,
        "samples_per_control_segment": 32,  # 3 segments * 32 + 1 = 97 rings
        "radial_segments": 16,
    },

    # C: increase only articulation/collision workload.
    "C": {
        "physics_links": 6,
        "samples_per_control_segment": 12,
        "radial_segments": 14,
    },

    # D: full heavier workload.
    "D": {
        "physics_links": 6,
        "samples_per_control_segment": 32,
        "radial_segments": 16,
    },
}

if SYNC_PRESET not in PRESETS:
    raise ValueError(
        f"Unknown SYNC_PRESET={SYNC_PRESET!r}. "
        f"Choose one of: {', '.join(PRESETS)}"
    )

SYNC_CONFIG = PRESETS[SYNC_PRESET]

OUTPUT_USD = str(
    OUTPUT_DIR / f"test_2bc_sync_scaling_{SYNC_PRESET.lower()}.usda"
)
...
class BranchSpec:
    ...
    # For Test 2B-B this scales the capsule's CYLINDRICAL SPINE length.
    # The two hemispherical caps extend the total axial size by 2*radius.
...
BRANCH_SPEC = BranchSpec(
    control_points=(
        ...
        (0.000000000, 0.101000000, 0.100000000),
        (0.034544034, 0.195908955, 0.100000000),
        (0.098822795, 0.272513399, 0.100000000),
    ),
    physics_links=SYNC_CONFIG["physics_links"],
    samples_per_control_segment=SYNC_CONFIG["samples_per_control_segment"],
    radial_segments=SYNC_CONFIG["radial_segments"],
    radius=RadiusProfile(
        # Match Test 0F: constant radius = 12 mm.
        base_radius=0.012,
        tip_radius=0.012,
        taper_start=0.0,
        taper_end=1.0,
        ...
        micro_variation_amplitude=0.0,
        micro_variation_cycles=0.0,
    ),

    # Test 0F: 0.05 kg per ~0.10 m link.
    linear_density_kg_per_m=0.50,

    # Keep only the newer 2B-B collision architecture.
    ...
    # Test 0F uses BLEND_HALF_WIDTH = 0.10 * 0.18 = 0.018 m.
    # Current formula:
    #   half_width = mean_link * skin_blend_fraction * 0.5
    # With ~0.10 m links, 0.36 gives ~0.018 m.
    skin_blend_fraction=0.36,
...
def create_rigid_link(stage, spec, centerline, physics, stem_path, index) -> str:
    ...
    This is the Test 2B-B configuration:
    ...
    for collider_i in range(spec.colliders_per_link):
        ...
        # IMPORTANT:
        # collider_length_scale now refers to the DESIRED TOTAL axial length
        # of the capsule, not just to its cylindrical spine.
        desired_total_length = (
            chord_length * spec.collider_length_scale
        )
        ...
        # A capsule's total axial size is:
        #
        #     spine_height + 2 * radius
        #
        # so derive the USD "height" (spine length) from the desired total.
        capsule_spine_height = (
            desired_total_length
            - 2.0 * collider_radius
        )

        if capsule_spine_height <= 1e-5:
            raise ValueError(
                "Capsule does not fit inside its allocated centerline "
                f"interval: link={index}, collider={collider_i}, "
                f"desired_total={desired_total_length:.6f} m, "
                f"diameter={2.0 * collider_radius:.6f} m. "
                "Reduce colliders_per_link or collider radius."
            )
        ...
        # Test 2B-B:
        # identical compound-proxy sampling as 2B-A, but use a capsule
        # instead of a flat-ended cylinder.
        #
        # In UsdGeom.Capsule, "height" is the cylindrical spine length;
        # the two hemispherical caps add one radius at each end.
        ...
        # Author an explicit local-space extent for the capsule.
        half_total = (
            0.5 * desired_total_length
        )
...
def build_stage(output_path: str = OUTPUT_USD) -> str:
    ...
    print("=" * 80)
    print("TEST 2B-C — SYNC SCALING")
    print("=" * 80)
    print(f"[OK] {output_path}")
    print(f"Sync preset             : {SYNC_PRESET}")
    print(
        f"  physics links         : "
        f"{SYNC_CONFIG['physics_links']}"
    )
    print(
        f"  visual rings target   : "
        f"{3 * SYNC_CONFIG['samples_per_control_segment'] + 1}"
    )
    print(
        f"  radial segments       : "
        f"{SYNC_CONFIG['radial_segments']}"
    )
    print()
    ...
    print(f"  capsules / link       : {BRANCH_SPEC.colliders_per_link}")
    print(f"  capsule shapes        : {len(links) * BRANCH_SPEC.colliders_per_link}")
    ...
    print()
    print("Main Test 2B-B check:")
    print("  same compound proxy sampling as 2B-A, but with rounded capsule ends.")
    print("  Expected here: 6 rigid links / 6 bones / 5 D6 / 12 capsule shapes.")
    print("=" * 80)
    return output_path
